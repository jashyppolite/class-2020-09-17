...
```{r}
raw <- read_excel("raw_data/class_enrollment_summary_by_term_9-17-2020.xlsx", skip = 3, col_names = TRUE)

en_fall_2020 <- raw %>%
  clean_names()  %>%
  select(course_title, u_grad) %>%
  filter(u_grad >= 10) %>%
  drop_na() %>%
  arrange(desc(u_grad))

top_course <- en_fall_2020%>%
  arrange(desc(u_grad)) %>%
  slice(1)
## got this info to use summarize_each from 
...
gov_50
en_fall_2020
# highest enrollment course: EC10

```
highest enrollment : ec10
gov50 amount of student: 151
...
en_fall_2020 <- raw %>%
  ...
  arrange(desc(u_grad)) %>%
  slice(12) %>%
  ggplot(aes(x = course_title, y = u_grad)) + geom_col()
